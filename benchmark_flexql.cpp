...
static const int INSERT_BATCH_SIZE = 10; // if you implement batch inserts in flexql, you can increase this for better performance

struct QueryStats {
    long long rows = 0;
};

static int count_rows_callback(void *data, int argc, char **argv) {
    (void)argc;
    (void)argv;
    // (void)azColName;
...
static bool query_rows(FlexQL *db, const string &sql, vector<string> &out_rows) {
    ...
    auto cb = [](void *data, int argc, char **argv) -> int {
        // (void)azColName;
